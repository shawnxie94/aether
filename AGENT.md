...
Aether is a Codex plugin for local style memory, prompt refinement, image generation bookkeeping, asset archiving, and style consistency review.

## Default Commands

Run commands from `plugins/aether` unless noted otherwise:
...
```bash
python -m json.tool schemas/style-card.schema.json >/dev/null
python -m json.tool schemas/prompt-record.schema.json >/dev/null
python -m json.tool schemas/generation-run.schema.json >/dev/null
```
...
- Style cards live in SQLite `styles`.
...
- Use `style-library` only for listing or inspecting existing styles.
- Use `style-capture` for reference images, screenshots, source-image prompts, or style sedimentation.
...
`/Users/shawn/.codex/plugins/cache/aether/aether/0.1.0`
...
- Keep schema changes backward-compatible and add migration guards for existing SQLite databases.
