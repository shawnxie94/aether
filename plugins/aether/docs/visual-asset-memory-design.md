...
当前 Aether 的核心资产是 style card。它适合复用整体风格，但如果想把一张图拆成多个可复用部分，并在后续自由组合，就需要从单一 Style Memory 升级为 Visual Asset Memory。
...
图片沉淀不再只生成一张 style card，而是生成候选素材模块。
...
Prompt 调优从“套用一个 style card”升级为“召回素材并组合”。
...
Style card 不需要删除。它可以升级为一种复合素材包：
...
也就是说，style card 后续可以变成：

```json
{
  "name": "Hand-Drawn Oil Pastel Anime",
  "linked_assets": [
    "style_xxx",
    "color_palette_xxx",
    "texture_xxx",
...
新增 `visual_assets` 概念，先不急着替换 style card。
...
让 `prompt-refine` 不只读取 style card，也能读取相关 visual assets。
