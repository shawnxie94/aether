...
| "沉淀", "记住", "保存风格", "分析风格", "判断是否已有", "保存素材", "分析素材" | `style-capture` |
...
6. If the user asks to "沉淀并生成" or similar, run `style-capture` first, then `prompt-refine` with the saved or selected visual assets, then ask before starting paid or external generation.
