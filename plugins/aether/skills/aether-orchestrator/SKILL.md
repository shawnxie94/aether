...
description: Default entrypoint for Aether plugin requests. Use for any Aether workflow, especially when the user provides images, source image prompts, fuzzy prompts, asks to capture reusable visual assets, browse persisted visual memory, refine prompts, generate images, edit generated images, or is ambiguous. Route text-only raw/fuzzy generation requests to prompt-refine first; the user's request to "generate" is not approval to skip refined-prompt confirmation. Route to image-generate only after an already refined/confirmed prompt, prompt record, or explicit auto-generate-after-refinement instruction. Image plus source-prompt inputs default to visual-asset-capture unless the user explicitly asks to generate or edit an image.
---
...
| Text-only raw/fuzzy image prompt plus explicit request to create, generate, render, or output a new image | `prompt-refine`, save and present the refined prompt, then stop and wait for user confirmation before `image-generate` |
...
5. When the user explicitly asks to generate and the input is a raw text prompt, save the prompt record, show the refined prompt, negative prompt, image params, and assumptions, then end the turn by asking the user to confirm or revise. The generation request itself is not confirmation.
6. Treat prompt refinement as a hard checkpoint before paid, external, irreversible, or provider-backed generation calls. Skip this confirmation only if the user explicitly says to auto-generate after refinement or asks to generate from an already confirmed prompt record.
...
- When presenting database objects to the user, hide internal IDs by default and show the stored readable name or a localized label. Only surface exact IDs when the user asks for low-level details or a command requires it.
