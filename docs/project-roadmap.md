...
1. 如果设置了环境变量 `AETHER_CONFIG_PATH`，优先读取该路径。
2. 否则读取当前工作区根目录的 `config.json`。
3. 如果未来支持多项目空间，可再读取当前工作区的 `.aether/config.json`。
4. 如果是全局默认配置，可读取 `~/.config/aether/config.json`，但它不应覆盖项目级配置。
